<template>
    <main class="grow pt-5" id="content" role="content">
     ...
     <div class="kt-container-fixed">
      <div class="flex flex-col items-stretch gap-7">
       <div class="flex items-center gap-3 w-full">
        ...
        <router-link class="kt-btn kt-btn-primary" to="/add-category">
         Add Category
        </router-link>
        <!--Product Filter-->
       ...
       <div class="" id="shop1_lists">
        <!-- begin: grid -->
        <div v-for="category in categories.data" :key="category.id" class="grid grid-cols-1 gap-5">
         <div class="kt-card">
          <div class="kt-card-content flex items-center flex-wrap justify-between p-2 pe-5 gap-4.5">
           ...
           <div class="flex items-center gap-1.5">
            <router-link class="kt-btn bg-yellow-500 text-black" :to="`/edit-category/${category.id}`">
             Edit
            </router-link>
            <button @click="deleteUnit(category.id)" class="kt-btn bg-orange-700 text-white kt-btn-outline ms-2 shrink-0">
             Delete
            </button>
           </div>
          </div>
     ...
     <div if="units.links" class="flex justify-center items-center space-x-2 mt-6">
        <button v-for="(link, index) in categories.links"
        :key="index"
        @click="fetchCategories(link.url)"
        :disabled="!link.url"
