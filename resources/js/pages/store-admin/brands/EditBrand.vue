          ...
          <div class="kt-card pb-2.5">
            <div class="kt-card-content grid gap-5">
              ...
              <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                ...
                <input v-model="form.name" class="kt-input" placeholder="Brand Name" type="text" />
              </div>
              ...
              <div v-if="imagePreview || form.existing_image" class="mt-2">
                <p class="text-sm text-gray-500">Current Image:</p>
                <img
                  :src="imagePreview ? imagePreview : '/storage/' + form.existing_image"
                  alt="Brand Image Preview"
                  class="h-32 mt-1 rounded shadow"
                />
                <div v-if="form.existing_image" class="mt-2">
                  <label class="text-sm">
...
export default {
  ...
  mounted() {
    ...
    axios.get(`/api/brands/${brandId}`).then((response) => {
      this.form.name = response.data.name;
      this.form.existing_image = response.data.brand_image ?? null;
    }).catch(error => {
  ...
  methods: {
    ...
    async updateBrand() {
      try {
        ...
        await axios.post(`/api/brands/${brandId}?_method=PUT`, formData, {
          headers: {
